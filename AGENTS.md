...
- The installer must refresh `KB Sleep`, `KB Dream`, `Khaos Brain System Update`, organization contribution, and organization maintenance automations under `$CODEX_HOME/automations`. It must permanently retire the exact legacy `kb-architect-pass` Skill and `kb-architect` automation on fresh installs and upgrades, without touching similarly named user assets.
