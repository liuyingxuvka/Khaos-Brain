...
- [x] 7.5 Implement rollback/resume/interruption/concurrent-change handling that restores one complete prior generation or completes the current one and keeps all five retained automations paused on failure.
...
## 8. Align Skills, prompts, SkillGuard, UI copy, and documentation
...
- [x] 8.2 Use SkillGuard maintenance supervision to compile each complete maintained inventory into current `.skillguard/contract-source.json`, `.skillguard/compiled-contract.json`, and `.skillguard/check-manifest.json`; remove every former authority shape and validate native execution depth.
- [ ] 8.3 Refresh installed Skill and automation projections transactionally, verify normalized/behavior-bearing source-install parity, preserve user pause state, and refresh the global router only after final managed-tree writes.
...
- [x] 8.5 Add static and behavior tests that fail on stale architecture wording, missing LogicGuard dependency declarations, Dream write authority, flat retrieval fallback, legacy schema guidance, former SkillGuard shapes, or installed-source drift.
...
- [ ] 9.5 Verify the terminal parent receipt consumes current terminal child receipts for LogicGuard package/API, model/projection, Sleep/Dream, retrieval/UI, migration/privacy, scale/performance, SkillGuard/source-install parity, installer, zero residuals, and existing Chaos Brain regressions.
- [ ] 9.6 Run `python scripts/install_codex_kb.py --json` and `python scripts/install_codex_kb.py --check --json` on the stable integration snapshot; prove SkillGuard, FlowGuard, and LogicGuard are each frozen to one exact digest for all long-assurance children even if a live editable-install pointer drifts; confirm current authority and all five retained automations' intended pause state; and rerun only the explicitly invalidated installation projection if packaging changed.
